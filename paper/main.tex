...
%   placeholder authors resolved, architecture diagram added.
...
We validate through a three-tier evaluation: 1,000 heuristic agent simulations, 100 LLM-backed agents (Claude Haiku), and four targeted quality tests judged by five models from four providers (Anthropic, Google, DeepSeek, Meta). Every one of the 20 individual judgments favored soul-enabled agents over stateless baselines, with the strongest gains in emotional continuity (9.7 vs.\ 1.9) and long-range recall (8.5 vs.\ 4.8). Inter-judge standard deviation stays below 0.8, indicating agreement across model families. The protocol, SDK, and all experimental code are open source.
\end{abstract}
...
The results support two claims: (1) memory enables factual recall (82\% hit rate vs.\ 0\%), and (2) combining memory with personality and emotional state tracking produces stronger responses than memory alone, particularly for emotional continuity (+7.8 points) and personality consistency (+4.0 points). We note that our baseline is a fully stateless agent with no memory or personality. A fairer comparison would pit \soul{} against a RAG-only system with equivalent memory capacity, which we leave to future work.

\paragraph{Where the gains come from.}
The largest improvement (emotional continuity, mean 9.7 vs.\ 1.9) comes from somatic markers: tracking the user's emotional arc across turns and referencing it in responses. Three of five judges gave the soul response a perfect 10/10 on this test. This suggests that emotional state tracking, though simple to implement, is an underexplored component in agent architectures. The personality consistency result (mean 9.0 vs.\ 5.0 with $\sigma = 0.2$) shows the tightest inter-judge agreement, indicating that persistent OCEAN traits produce reliably distinct agent behaviors. However, we use OCEAN as an engineering tool for consistent persona generation, not as a validated psychometric instrument for non-human agents.
...
\paragraph{Weak baseline.} Our primary comparison is against a fully stateless agent. A stronger evaluation would compare \soul{} against RAG-only memory (with equivalent storage), prompt-injected personality (without persistence), and production systems like Mem0~\cite{chhablani2025mem0}. The current results show that \textit{having} identity components helps; they do not isolate \textit{which} components matter most.
...
\paragraph{No component ablation.} The heuristic cognitive engine produced identical metrics across ablation conditions, preventing us from isolating the contribution of significance scoring, somatic markers, and bonds individually. LLM-backed ablation with longer conversation horizons is needed.
